...
func (s *Ed448InternalSuite) Test_GaloisField_Bias32(c *C) {
	tmp1 := [128]byte{0x01}
	tmp2 := [128]byte{0xff, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfc, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f, 0xfe, 0xff, 0xff, 0x1f}

	x := NewGaloisField448FromBytes(tmp1[:])
	exp := NewGaloisField448FromBytes(tmp2[:])

	y := Bias32(x, 2)

	c.Assert(y.limbs32(), DeepEquals, exp.limbs32())
	c.Assert(y.Limb.Size(), Equals, 128)
